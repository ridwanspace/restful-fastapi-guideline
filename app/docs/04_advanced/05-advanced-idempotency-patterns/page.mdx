...
sequenceDiagram
    ...
    participant Store as 🗄️ Idempotency Store

    note over Client: 1. Generate unique Idempotency-Key
    Client->>Server: 2. Send POST request with Idempotency-Key

    alt First Request (Key is New)
        Server->>Store: 3a. Check for key (not found)
        Store-->>Server: 4a. Key is new
        Server->>Store: 5a. Store key as "processing"
        Store-->>Server: 6a. Acknowledged
        Server->>Server: 7a. Process request...
        Server->>Store: 8a. Update key with final response
        Store-->>Server: 9a. Acknowledged
        Server-->>Client: 10a. Return new response
    else Subsequent Request (Key is Seen)
        Server->>Store: 3b. Check for key (found)
        Store-->>Server: 4b. Return stored response
        Server-->>Client: 5b. Return stored response immediately
    end
```
