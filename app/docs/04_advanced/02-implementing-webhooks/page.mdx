...
```mermaid
graph TD
    A[🏰 Kingdom A<br/>(Your FastAPI App)] -->|1. Event Occurs<br/>(e.g., New Order Created)| B((🔔))
    B -- 2. Trigger Webhook --> C(📮 Royal Messenger Dispatched<br/>HTTP POST Request)
    C -- 3. Travels to Destination --> D[🏰 Kingdom B<br/>(External Service)]
    D -- 4. Receives Message --> E{✅ Acknowledges Receipt<br/>(200 OK Response)}
