...
```mermaid
graph TD
    subgraph "<br>Restaurant Dining Experience"
        %% Invisible node for spacing
        invisible_node[ ]
        invisible_node ~~~ A[🧑 Customer<br/>You arrive hungry]
        A --> B[📋 Menu<br/>Browse options]
        B --> C[🤵 Waiter<br/>Takes your order]
        C --> D[👨‍🍳 Kitchen<br/>Prepares food]
        D --> E[🍽️ Plated Dish<br/>Food is ready]
        E --> C
        C --> F[😋 Happy Customer<br/>Enjoys meal]
    end
    
    style invisible_node fill:none,stroke:none
    style C fill:#e1f5fe,stroke:#0277bd,stroke-width:3px,color:#3b3b3b
    style A fill:#f3e5f5,color:#3b3b3b
...
graph LR
    ...
    style B fill:#e1f5fe,stroke:#0277bd,stroke-width:3px,color:#3b3b3b
```
...
### 1. Weather Apps 🌤️ - The Weather Information Restaurant
...
```mermaid
sequenceDiagram
    participant You as 📱 Your Weather App
    participant API as 🌐 Weather API<br/>(The Waiter)
    participant Service as ☁️ Weather Service<br/>(The Kitchen)
    participant Sensors as 📡 Weather Stations<br/>(Fresh Ingredients)
    ...
    Sensors-->>Service: Temp: 72°F, Humidity: 65%
...
graph TD
    ...
    subgraph "Behind the Scenes"
        E --> H[🖼️ Generate Thumbnails]
        E --> I[🏷️ Process Hashtags]
...
stateDiagram-v2
    ...
    ShoppingCart --> Checkout: Proceed to pay<br>
    ...
    state Checkout {
        ...
        APICall --> Success: Approved ✅
        APICall --> Failed: Declined ❌
    }
...
### 1. **Separation of Concerns** 🏗️ - The Multi-Restaurant Kitchen
...
graph TD
    subgraph "Different Dining Experiences"
        %% Invisible node for spacing
        invisible_node[ ]
        invisible_node ~~~ A[📱 Mobile App<br/>Quick Service Counter]
        B[💻 Web App<br/>Full Service Restaurant]
        C[⌚ Smart Watch<br/>Drive-Through]
        D[🎮 Gaming Console<br/>Delivery Service]
    end
    
    subgraph "Shared Kitchen (API)"
        E[🌐 REST API<br/>Order Management]
    end
    
    subgraph "Kitchen Operations"
        F[👨‍🍳 Business Logic<br/>Recipe Execution]
        G[🗄️ Database<br/>Ingredient Storage]
        H[🔐 Security<br/>Health & Safety]
    end
    ...
    style E fill:#e1f5fe,stroke:#0277bd,stroke-width:3px,color:#3b3b3b
```
...
### 2. **Reusability** ♻️ - The Franchise Model
...
graph LR
    subgraph "Google Maps API Kitchen"
        A[🗺️ Google Maps API<br/>Master Recipe Book]
    end
    ...
    style A fill:#e1f5fe,stroke:#0277bd,stroke-width:3px,color:#3b3b3b
```
...
stateDiagram-v2
    ...
    state "API Security Layers" as APISecurityLayers {
    ...
    Authorization --> Rejected: No permission ❌
    Validation --> Rejected: Bad request ❌
    RateLimiting --> Rejected: Too many requests ❌
...
### 1. **Client-Server Separation** 📱↔️🖥️ - The Restaurant and Home Delivery
...
graph LR
    subgraph "Clients (Where You Eat)"
        A1[🏠 Home<br/>Mobile App]
        A2[🏢 Office<br/>Web Browser]
        A3[🚗 Car<br/>Voice Assistant]
    end
    
    subgraph "Internet (Delivery Service)"
        B[(🌐 HTTP/HTTPS<br/>The Delivery Network)]
    end
    
    subgraph "Server (The Restaurant)"
        C[🍳 Kitchen<br/>Business Logic]
        D[🗄️ Storage<br/>Database]
    end
...
graph TD
    subgraph "1. Resource Identification"
        A[🏷️ Everything has a unique address<br/>/restaurants/123<br/>/restaurants/123/menu<br/>/restaurants/123/reviews]
    end
    
    subgraph "2. Resource Manipulation"
        B[🔄 Use standard methods<br/>GET - Read<br/>POST - Create<br/>PUT - Update<br/>DELETE - Remove]
    end
    
    subgraph "3. Self-Descriptive Messages"
        C[📦 Complete information<br/>Headers describe content<br/>Status codes explain results<br/>Body contains data]
    end
    
    subgraph "4. HATEOAS*"
        D[🔗 Links to related actions<br/>Like a 'You might also like' section]
    end
...
async def update_book(book_id: int, updates: BookUpdate):
    """
    PATCH = "Update just these parts" ✏️
...
### 4. **Layered System** 🏗️ - The Restaurant Service Layers
...
graph TB
    ...
    subgraph "Hidden Layers"
        ...
        C[🚪 API Gateway<br/>Entry Point]
        D[🔒 Auth Service<br/>Security Check]
        E[⚖️ Load Balancer<br/>Traffic Distribution]
        F[💾 Cache Layer<br/>Quick Responses]
        G[🏢 Application Servers<br/>Business Logic]
        H[🗄️ Database Cluster<br/>Data Storage]
    end
...
sequenceDiagram
    ...
    Cache->>Cache: Check cache: Not found ❌
...
<div className="mt-12 rounded-xl p-5 text-center bg-sky-100/50 dark:bg-sky-400/10">
