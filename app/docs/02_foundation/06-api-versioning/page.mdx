...
```mermaid
graph TD
    A[🧑‍💼 Client App] --> B{Which Menu Edition?}
    B -->|Classic Menu| C[📋 API v1<br/>Basic endpoints]
    B -->|Updated Menu| D[📋 API v2<br/>Enhanced features]
    B -->|Premium Menu| E[📋 API v3<br/>Latest features]
    
    C --> F[🍽️ Simple Response<br/>Core data only]
    D --> G[🍽️ Enhanced Response<br/>Core + metadata]
    E --> H[🍽️ Rich Response<br/>Full feature set]
...
### 🏗️ The Foundation Benefits

- **🛡️ Backward Compatibility**: Like keeping classic dishes on the menu - existing customers (clients) can still order their favorites
- **🎛️ Controlled Evolution**: Roll out new features gradually, like introducing seasonal specials without removing the core menu
- **⏰ Client Flexibility**: Different clients can "upgrade their dining experience" (adopt new versions) at their own pace
...
graph LR
    ...
    B -->|/api/v1/| C[🍽️ Classic Menu<br/>Basic User Data]
    B -->|/api/v2/| D[🍽️ Premium Menu<br/>Enhanced User Data]
    
    C --> E[📋 UserV1<br/>id, name, email]
    D --> F[📋 UserV2<br/>id, name, email<br/>+ created_at<br/>+ profile_picture]
...
1. **🏗️ Built two dining areas** - Classic (v1) and Premium (v2) sections
2. **📋 Created different menus** - Simple dishes vs gourmet offerings  
3. **👨‍🍳 Trained specialized staff** - Different chefs for different experiences
...
```mermaid
graph TD
    A[📱 Client Request] --> B[🗣️ Header Check<br/>Accept-Version: v2]
    B --> C{Which Recipe?}
    C -->|v1 Header| D[🍽️ Classic Recipe<br/>Basic User Data]
    C -->|v2 Header| E[🍽️ Premium Recipe<br/>Enhanced User Data]
    C -->|No Header| F[🍽️ Default Recipe<br/>Falls back to v1]
...
2. **🍽️ Created flexible service** - Same dining table, different experiences
...
- **👁️ Less Visible**: Version isn't obvious from URL - need to check headers
...
## 🎟️ Query Parameter Versioning: The Ticket Booth Approach
...
```mermaid
graph TD
    A[📱 Client Request] --> B[🎟️ Check Ticket Type<br/>?version=v2]
    B --> C{Which Experience?}
    C -->|?version=v1| D[🎠 Basic Ride<br/>Standard Features]
    C -->|?version=v2| E[🎢 Premium Ride<br/>Enhanced Features]
    C -->|No Ticket| F[🎠 Default Ride<br/>Falls back to v1]
    
    D --> G[📋 Basic Response]
    E --> H[📋 Premium Response]  
    F --> G
...
1. **🎟️ Set up a ticket booth** - Customers can choose their experience level via query parameters
2. **🎠 Created tiered attractions** - Different experiences based on ticket type
3. **🗺️ Built a park map** - Easy navigation with clear upgrade paths
...
- **👁️ Highly Visible**: Version is clearly visible in the URL - easy to see and share
...
```mermaid
graph TD
    A[🤔 Which Versioning Strategy?] --> B{Primary Concern?}
    
    B -->|URL Clarity| C[🌟 URL-Based<br/>✅ Clear paths<br/>✅ Easy debugging<br/>❌ URL proliferation]
    B -->|Clean URLs| D[🎧 Header-Based<br/>✅ Clean URLs<br/>✅ Flexible<br/>❌ Less visible]
    B -->|Simplicity| E[🎟️ Query Parameter<br/>✅ Visible & simple<br/>✅ Easy testing<br/>❌ URL pollution]
    B -->|Standards| F[📡 Content Negotiation<br/>✅ HTTP standard<br/>✅ Sophisticated<br/>❌ Complex setup]
...
4. **⚡ Smart Detection**: Implement the smart version detector
...
- **🍽️ Keep classics available** (maintain backward compatibility)
...
- **⏰ Announce changes in advance** (deprecation warnings)
