...
```mermaid
graph TD
    A[🧑‍💼 Customer arrives<br/>HTTP Request] --> B[🏪 Restaurant Host<br/>FastAPI receives request]
    B --> C{📋 Order Type?<br/>Parameter Classification}
    
    C --> D[🚪 Table Request<br/>Path Parameters<br/>/table/5]
    C --> E[📝 Special Instructions<br/>Query Parameters<br/>?dietary=vegan&spice=mild]
    C --> F[🍽️ Custom Order<br/>Request Body<br/>Complex meal details]
    C --> G[🎫 VIP Card<br/>Headers & Cookies<br/>Authentication]
    
    D --> H[👨‍🍳 Kitchen processes<br/>Function execution]
    E --> H
    F --> H
    G --> H
    
    H --> I[🍽️ Meal ready<br/>JSON Response]
    I --> J[😋 Customer satisfied<br/>Response received]
...
## 🏗️ The Four Pillars of Request Handling
...
⚠️ **Warning**: Path parameters are always required. If a customer doesn't specify a table number, they can't be seated!
...
```mermaid
graph TD
    A[📝 Query Parameters<br/>?param=value&other=123] --> B[🔍 Filtering<br/>category=appetizer]
    A --> C[📄 Pagination<br/>page=2&limit=10]
    A --> D[📊 Sorting<br/>sort=price&order=asc]
    A --> E[🔎 Search<br/>q=chicken+tikka]
    
    B --> F[🎯 Targeted Results]
    C --> F
    D --> F
    E --> F
...
async def search_menu_items(
    ...
    # 🌶️ Dietary preferences - like telling the waiter your restrictions
    ...
    # ⭐ Rating filter - only show highly rated dishes
...
⚠️ **Warning**: Validate parameter combinations that don't make sense, like `min_price > max_price`.
...
## 3. 🍽️ Request Body: The Custom Order System
...
```mermaid
graph TD
    A[🧑‍💼 Customer places<br/>complex order] --> B[📝 Order form<br/>Request Body JSON]
    B --> C[✅ Pydantic validation<br/>Check all requirements]
    C --> D{🔍 Valid order?}
    
    D -->|✅ Yes| E[👨‍🍳 Kitchen processes<br/>Function execution]
    D -->|❌ No| F[🚫 Reject order<br/>Validation error]
    
    E --> G[🍽️ Dish created<br/>Success response]
    F --> H[📋 Explain what's wrong<br/>Error details]
...
⚠️ **Warning**: Always validate complex data relationships. A dessert shouldn't be "extra spicy"!
...
```mermaid
graph TD
    A[🧑‍💼 Customer visits<br/>with VIP card] --> B[🎫 Show credentials<br/>Headers sent]
    A --> C[🍪 Loyalty history<br/>Cookies sent]
    
    B --> D[✅ Verify identity<br/>Authentication check]
    C --> E[📋 Check preferences<br/>Personalization]
    
    D --> F{🔒 Valid customer?}
    F -->|✅ Yes| G[🏪 Welcome VIP<br/>Process request]
    F -->|❌ No| H[🚫 Access denied<br/>Authentication error]
    
    E --> G
    G --> I[🍽️ Personalized service<br/>Custom response]
    I --> J[🍪 Update loyalty card<br/>Set response cookies]
...
⚠️ **Warning**: Always validate and sanitize header values. Never trust user-provided headers for security decisions!
...
```mermaid
graph TD
    A[📱 Customer selects<br/>photos to upload] --> B[📋 Form submission<br/>with file data]
    B --> C[🔍 Validate file type<br/>Size and format check]
    C --> D{✅ Valid file?}
    
    D -->|✅ Yes| E[💾 Save file securely<br/>Generate unique filename]
    D -->|❌ No| F[🚫 Reject file<br/>Explain requirements]
    
    E --> G[🖼️ Process image<br/>Resize, optimize]
    G --> H[📁 Store in gallery<br/>Update database]
    H --> I[✅ Success response<br/>File URL returned]
...
⚠️ **Warning**: Generate unique filenames to prevent naming conflicts and security issues.
...
```mermaid
graph TD
    A[🧑‍💼 Customer request<br/>to multiple endpoints] --> B[🔧 Common requirements<br/>Pagination, Auth, etc.]
    
    B --> C[👨‍🍳 Prep Cook<br/>Dependency function]
    C --> D[📋 Validate parameters<br/>Apply business rules]
    D --> E[🍽️ Prepared ingredients<br/>Processed parameters]
    
    E --> F[🏪 Endpoint 1<br/>Menu items]
    E --> G[🏪 Endpoint 2<br/>Orders]
    E --> H[🏪 Endpoint 3<br/>Reviews]
    
    F --> I[📊 Consistent results<br/>Same format everywhere]
    G --> I
    H --> I
...
async def get_popular_dishes(
    ...
    # ⭐ Additional filters specific to this endpoint
    min_rating: Annotated[
        float,
...
⚠️ **Warning**: Don't over-engineer dependencies. Keep them focused on single responsibilities.
...
### 2. ⚡ Validate Early and Clearly: The Quality Control Chef
...
async def search_menu_items(
    ...
    # 🌶️ Dietary preferences with clear options
...
async def get_dish_reviews(
    ...
    # 🏗️ Build filters dynamically based on what's provided
